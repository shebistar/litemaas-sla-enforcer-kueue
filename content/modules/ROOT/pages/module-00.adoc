...
oc label --overwrite node \
    --selector=nvidia.com/gpu.product=NVIDIA-L4-SHARED \
...
====
The selector value ``nvidia.com/gpu.product=NVIDIA-L4-SHARED`` must match the GPU product name as labeled by the GPU Operator's Node Feature Discovery (NFD) component.
====
...
[source,bash]
----
oc get node --selector=nvidia.com/gpu.product=NVIDIA-L4-SHARED -o json \
