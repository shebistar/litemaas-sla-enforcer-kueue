...
. **GPU Worker Node:** You need at least one worker node with a time-sliced NVIDIA L4 GPU, as configured in the previous lab. On AWS, a `g5.2xlarge` instance is suitable.
...
. **ResourceFlavor:** We will define a `ResourceFlavor` that targets our specific time-sliced L4 GPUs using a `nodeLabel` selector. This ensures that all GPU requests are scheduled on the correct hardware.
...
In this scenario, the label selector is `nvidia.com/gpu.product: NVIDIA-L4-SHARED`:
...
kind: ResourceFlavor
metadata:
  name: nvidia-l4-shared
spec:
  nodeLabels:
    nvidia.com/gpu.product: NVIDIA-L4-SHARED
EOF
----
